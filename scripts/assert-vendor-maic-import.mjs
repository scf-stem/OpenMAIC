 ...
 *
 * The app loads the parser at runtime from `/vendor/maic-import/index.js` — a
 * gitignored build artifact that `scripts/sync-maic-import.mjs` copies into
...
const entry = path.join(root, 'public/vendor/maic-import/index.js');
...
try {
  ...
  console.error('[assert-vendor] It is loaded at runtime via /vendor/maic-import/index.js');
  console.error('[assert-vendor] and is produced by the postinstall sync step. To fix:');
  console.error('[assert-vendor]   pnpm --filter @maic/importer build && pnpm run sync:maic-import');
