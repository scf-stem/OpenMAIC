...
describe('mergeAssistantParts (chronological)', () => {
  it('renders wrap-up text BELOW the tool card (turn order preserved)', () => {
    const turns: PiPart[][] = [
      [{ type: 'toolCall', id: 't1', name: 'regenerate_scene_actions', arguments: { sceneId: 's' } }],
  ...
  it('attaches tool results by toolCallId', () => {
    ...
    const results = new Map([['t1', { result: { details: { sceneId: 's', actions: [1] } }, isError: false }]]);
    const parts = mergeAssistantParts({ turns, toolResults: results, error: '' });
    expect(parts[0]).toMatchObject({ type: 'tool-call', result: { details: { sceneId: 's', actions: [1] } } });
  });
  ...
  it('drops empty text and falls back to a single empty part', () => {
    expect(
      mergeAssistantParts({ turns: [[{ type: 'text', text: '' }]], toolResults: NO_RESULTS, error: '' }),
...
describe('tool_execution_end empty-actions guard', () => {
  ...
  it('DOES call updateScene when actions has at least one entry', () => {
    const calls = simulateToolExecutionEnd({ sceneId: 's1', actions: [{ type: 'speech', id: 'a1' }] });
  ...
  it('does NOT call updateScene when sceneId is missing', () => {
    expect(simulateToolExecutionEnd({ sceneId: undefined, actions: [{ id: 'a1' }] })).toHaveLength(0);
  });
});
